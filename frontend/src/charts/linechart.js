...
const LineChart = () => {
  ...
  const fetchDataFromApi = async () => {
    if (selectedCompostBinId) {
      const apiUrl = `http://vps-3744359-x.dattaweb.com:8080/api/compost_bins/${selectedCompostBinId}/measurements`;
  ...
  useEffect(() => {
    ...
    const fetchFirstCompostBinId = async () => {
      try {
        const response = await fetch("http://http://vps-3744359-x.dattaweb.com:8080/api/compost_bins/all_ids");
        if (!response.ok) {
